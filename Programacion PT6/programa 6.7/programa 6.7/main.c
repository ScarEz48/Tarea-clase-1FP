...
/* Fabrica de lacteos.
El programa, al recibir como datos las ventas mensuales de diferentes productos,
obtiene informacion estadistica valiosa para la empresa. */

void Lectura1(int [15] [12]);
void Lectura1(float, int);
...
{
    A[MES-1][PRO-1] += CAN;
    printf("Ingrese mes, tipo de producto y cantidad vendidas: ");
    scanf("%d %d %d", &MES, &PRO, &CAN);
}
}
...
/* Esta funcion se utiliza para calcular el monto de venta anual de cada uno de los productos. Observa
que el resultado se almacena en en un arreglo unidimensional que se utilizara posteriormente. */
{
    int I, J, SUM;
    printf("\n");
    for (I=0; I< F; I++)
...
/* Esta funcion se utiliza para calcular el monto total de ventas de la fabrica. */
{
    ...
    printf("\n\nTipo de producto mas vendido: %d \t  Ventas: %2.f", TPR + 1, VEN);
}
