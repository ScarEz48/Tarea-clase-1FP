...
El programa maneja informacion sobre ventas, inventariom reabastecimiento y nuevos productos de una comercializadora farmaceutica. */
...
void main(void)
{
    ...
    while (OPE)
    {
        switch (OPE)
        {
            ...
            /* Se pasa el parametro por referencia, porque se puede modificar el numero
...
/* Esta funcion se utiliza para leer un arreglo unidimensional de tipo estructura producto de T elemento. */
{
    int I;
    for (I=0; I<T; I++)
    {
...
productos y cantidades, el fin de datos por el cero. Ademas de obtener el total
de las ventas, se actualiza el inventario. */
{
    int CLA, CAN, I, RES;
    float TOT, PAR;
    ...
    while (CLA)
    {
        ...
        printf("\nIngrese la siguiente clava del producto -0 para salir-:");
        scanf("%d", &CLA);
    }
...
insertar un nuevo producto haya que mover los elementos del arreglo para que continuen ordenados.*/
{
    int CLA, I, J;
    ...
    while ((*T < 30) && (CLA))
    {
        I = 0;
        while ((I < *T) && (A[I].clave < CLA))
        /* Busqueda de la posicion que le corresponda a CLA en el arreglo. */
        I++;
        ...
        {
            for (J=*T; J>I; J--)
            /* Se inserta el nuevo producto en el arreglo. Se mueven una posicion a la
            derecha los elementos del arreglo que tengan una clave de producto mayor a la ingresada. */
            A[J] = A[J-1];
            A[I].clave = CLA;
            printf("\tNombre:");
            fflush(stdin);
...
/* Esta funcion se utiliza para escribir la informacion almacenada en -el Inventario- es un arreglo unidimensional de tipo
