...
matpro cal[5];               /* Observa que cal es un arreglo unidimensional de tipo estructura
...
while (TAM > 50 || TAM < 1); /* Se verifica que el tamaño del arreglo seacorrecto. */
Lectura(ALU, TAM);
F1(ALU, TAM);
F2(ALU, TAM);
F3(ALU, TAM);
...
/* Esta función se utiliza para obtener la matrícula y el promedio general decada alumno. */
{
int I, J;
float SUM;
for (I=0; I<T; I++)
